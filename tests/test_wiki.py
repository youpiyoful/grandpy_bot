...
def test_find_data_about_place(monkeypatch):
    ...
    assert resp == {
        "wiki_response": "Openclassrooms est un site web d'apprentissage."
    }
    assert (
        resp.get("wiki_response")
        == "Openclassrooms est un site web d'apprentissage."
    )
...
def test_find_any_data_about_place(monkeypatch):
    ...
    page_error_response = {
        'error': 'Page id "tour eifel grandes" does not match any pages. Try another id!'
    }
